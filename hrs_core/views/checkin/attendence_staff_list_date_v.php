                    ...
                    <div class="widget-content">
                        <div>
                            <form id="form_search" name="search_form" action>
                                ...
                                <input type="text" class="form-control span2 form_datetime" name="start_date" placeholder="开始日期"<?=$filters['start_date']?' value="'.$filters['start_date'].'"':''?>>
                                <input type="text" class="form-control span2 form_datetime" name="end_date" placeholder="结束日期"<?=$filters['end_date']?' value="'.$filters['end_date'].'"':''?>>
                                ...
                                <input type="button" class="form-control btn btn-warning" style="margin-bottom: 10px;" value="导出明细" id="expdetail">
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="center"><?=$row['sum']['work_day']?></td>
                                    ...
                                    <td class="center"><?=$row['sum']['leave_time']?></td>
                                    <td class="center"><?=$row['sum']['holiday_time']?></td>
                                    <td class="center"><?=$row['sum']['error_time']?></td>
                                    <td class="center"><?=$row['sum']['late_time']?></td>
                                    <td class="center"><?=$row['sum']['first_late']?></td>
                                    <td class="center"><?=$row['sum']['other_late']?></td>
                                    <td class="center">
                                        <a class="btn btn-success" href="/checkin/attendence/detaildate?staff_id=<?=$row['id']?>&start_date=<?=$filters['start_date']?>&end_date=<?=$filters['end_date']?>">明细</button>
                                    </td>
                                </tr>
                                <?php endforeach?>
                            </tbody>
...
$("#expdata").on("click", function(){
    $('#form_search').attr("action", "/checkin/attendence/expdatadate");
    $('#form_search').submit();
})

$("#search_btn").on("click", function(){
    $('#form_search').attr("action", "/checkin/attendence/staffdate");
    $('#form_search').submit();
})

$("#expdetail").on("click", function(){
    $('#form_search').attr("action", "/checkin/attendence/expalldetaildate");
    $('#form_search').submit();
})
</script>
