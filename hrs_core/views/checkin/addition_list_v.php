                        ...
                        <div>
                            <form>
                                ...
                                <select name="type" class="span2">
                                    ...
                                    <option value="1"<?=$filters['type']==1?' selected':''?>>中午</option>
                                    <option value="2"<?=$filters['type']==2?' selected':''?>>晚上</option>
                                </select>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="center">
                                        <button class="btn btn-success add_addition">新增</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="widget-content nopadding">
            <form action="#" method="get" class="form-horizontal">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" id="addition" name="addition" class="form_datetime" />
                    </div>
                </div>
                <div class="control-group">
...
$('.add_addition').on('click', function(){
    ...
    $('#addition_edit_form').find('#target').val(0);
    $('#addition_edit_form').find('#dept_id').val(0);
    $('#addition_edit_form').find('#staff_code').val('');
    $('#addition_edit_form').find('#addition').val('');
...
$('.edit_addition').on('click', function(){
    var addition_id = $(this).val();
    $.ajax({ 
        url: "/checkin/addition/one",
        data: {"addition_id":addition_id},
        dataType: 'json',
        type: 'POST',
        success: function(result){
            if(result.status == 1){
                $('#addition_edit_form').find('h5').text('修改');
                $('#addition_edit_form').find('#addition_id').val(result.data.id);
                $('#addition_edit_form').find('#target').val(result.data.target);
                $('#addition_edit_form').find('#dept_id').val(result.data.dept_id);
                $('#addition_edit_form').find('#staff_code').val(result.data.staff_code);
                $('#addition_edit_form').find('#addition').val(result.data.addition);
                $('#addition_edit_form').find('#hours').val(result.data.hours);
                $('#addition_edit_form').find('#type').val(result.data.type);
                $('#addition_edit_form').find('#is_enable').val(result.data.is_enable);
                $('#addition_edit_form').modal('show');
            }else{
