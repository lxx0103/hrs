                        ...
                        <div>
                            <form id="form_search">
                                ...
                                <select name="type" class="span2">
                                    ...
                                    <option value="4"<?=$filters['type']==4?' selected':''?>>旷工</option>
                                    <option value="5"<?=$filters['type']==5?' selected':''?>>其他</option>
                                </select>                             
                                <select name="is_enable" class="span2">
                                    <option value="2">是否启用</option>
                                    <option value="1"<?=$filters['is_enable']==1?' selected':''?>>是</option>
                                    <option value="0"<?=$filters['is_enable']==0?' selected':''?>>否</option>
                                </select>
                                ...
                                <input type="button" class="form-control btn btn-success" style="margin-bottom: 10px;" value="导出" id="expdata">
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="widget-box">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="center"><?=$row['holiday_end']?></td>
                                    <td class="center"><?=$row['hours']?></td>
                                    <td class="center"><?=$row['type']==1?'放假':($row['type']==2?'事假':($row['type']==3?'病假':($row['type']==4?'旷工':'其他')))?></td>
                    ...
                    <div class="dataTables_paginate" style="padding-bottom: 5px;">
                        ...
                        <span> 
                            ...
                            <?php if(ceil($total/$filters['page_size'])>=$filters['page']+1): ?>
                            <a tabindex="0" class="fg-button ui-button ui-state-default" href="<?=$query_str.'page='.($filters['page']+1)?>"><?=$filters['page']+1?></a>
                            <?php endif;?>
                            <?php if(ceil($total/$filters['page_size'])>=$filters['page']+2): ?>
                            <a tabindex="0" class="fg-button ui-button ui-state-default" href="<?=$query_str.'page='.($filters['page']+2)?>"><?=$filters['page']+2?></a>
                            <?php endif;?>
                        </span>
                    </div>
        ...
        <div class="widget-content nopadding">
            <form action="#" method="get" class="form-horizontal">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" id="holiday_end" name="holiday_end" class="form_datetime" />
                    </div>
                </div>
                <div class="control-group">
...
$('.add_holiday').on('click', function(){
    ...
    $('#holiday_edit_form').find('#holiday_end').val('');
    ...
    $.ajax({ 
        ...
        success: function(result){
            if(result.status == 1){
                ...
                $('#holiday_edit_form').find('#holiday_end').val(result.data.holiday_end);
...
$('#save_holiday').on('click', function(){
    $.ajax({ 
        ...
        data: {"target":$("#target").val(), "dept_id":$("#dept_id").val(), "staff_code":$("#staff_code").val(), "holiday":$("#holiday").val(), "holiday_end":$("#holiday_end").val(), "hours":$("#hours").val(), "type":$("#type").val(), "is_enable":$("#is_enable").val(), "holiday_id":$("#holiday_id").val()},
        dataType: 'json',
        type: 'POST',
        success: function(result){            
            if(result.status == 1){
...
$("#expdata").on("click", function(){
    $('#form_search').attr("action", "/checkin/holiday/expholiday");
    $('#form_search').submit();
})
</script>
